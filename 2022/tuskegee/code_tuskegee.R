...
airmen_sum <- airmen %>% 
  group_by(name, last_name, first_name, pilot_type) %>% 
  summarise(downs = sum(tally_num, na.rm = TRUE)) %>% 
  ungroup() %>% 
  mutate(downs = ceiling(downs)) %>% 
  arrange(desc(downs))

airmen_downs <- airmen_sum %>% 
  filter(downs > 0)

ggplot(airmen_downs) +
  geom_dotplot(aes(x = downs),
               binwidth = 1/19)
